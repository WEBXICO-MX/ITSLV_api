			...
			<tr>
				...
				<td><form:select path="division_id.id">
						...
						<c:forEach items="${listDivision}" var="division">
							<c:choose>
								<c:when
									test="${division.id == carrera.getDivision_id().getId()}">
									<option value="${division.id}" selected="selected">${division.nombre}</option>
								</c:when>
								<c:otherwise>
